...
class Robot(object):
    ...
    def getObjectPose(self, obj_handle):
        sim_ret, obj_position = vrep.simxGetObjectPosition(self.sim_client, object_handle, -1, vrep.simx_opmode_blocking)
        sim_ret, obj_orientation =vrep.simxGetObjectOrientation(self.sim_client, object_handle, -1, vrep.simx_opmode_blocking)
    ...
    def check(self,obj_handle,baseobj,z_value,i):
        sim_ret, object_position = vrep.simxGetObjectPosition(self.sim_client,obj_handle, -1, vrep.simx_opmode_blocking)
        sim_ret, object_position1 = vrep.simxGetObjectPosition(self.sim_client,baseobj, -1, vrep.simx_opmode_blocking)
        ok=object_position
        object_position[2]+=i*(z_value+0.05)
        if(object_position==object_position1):
         return True,None;
        else:
         return False,ok;
    ...
    def move_to(self, tool_position,tool):
        ...
        if self.is_sim:
            ...
            move_magnitude = np.linalg.norm(move_direction)
            move_step = 0.008*move_direction/move_magnitude
            num_move_steps = int(np.floor(move_magnitude/0.02))
            
             
            x=num_move_steps
            for step_iter in range(x):
...
# Primitives ----------------------------------------------------------
    ...
    def grasp(self, position,heightmap_rotation_angle, workspace_limits,set_position,orientation,set_flag=0):
        ...
        if self.is_sim:
            ...
            if set_flag==1:
                 #vrep.simxSetObjectPosition(self.sim_client,grasped_object_handle,-1,set_position,vrep.simx_opmode_blocking)
                  pose=self.object_pose(set_position,orientation)
                  self.push_to(pose, single_step=False)
                  print(" Moving the grasped object ")                 
                  self.open_gripper()
                  set_position=(set_position[0],set_position[1], set_position[2] + 0.05)
    ...
    def push(self, position, heightmap_rotation_angle, workspace_limits):
        ...
        if self.is_sim:
            ...
            self.close_gripper()
            #positions=[]
            #position1 = (position[0]+0.2, position[1], position[2])#left
            #position2 = (position[0]-0.2, position[1], position[2])#right
            #position3 = (position[0], position[1]+0.2, position[2])#top
            #position4 = (position[0], position[1]-0.2, position[2])#down
            #positions=[position1,position2,position3,position4]
            #position_k=random.choice(positions)
...
def main(args):
    ...
    # --------------- Setup options ---------------
        is_sim = True
        obj_mesh_dir = '/home/uthira/visual-pushing-grasping/objects/tower' if is_sim else None # Directory containing 3D mesh files (.obj) of objects to be added to simulation
        ...
        path='/home/uthira/visual-pushing-grasping/img'
        cv2.imwrite(os.path.join(path,"color image.jpg"),color_img)
        ...
        for i in range(1,4):
         ...
         #best_pix_z=random.randrange(1,3, 1)
